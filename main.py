...
def drawMenu():
    ...
    title_line = "║" + " " * padding + title + " " * (width - len(title) - padding) + "║"
    ...
    for line in lines[1:]:
        if line == "":
            ...
            if line.startswith("1."):
                console.print(menu_line, style="cyan")  # Calculate - cyan
            elif line.startswith("2."):
                console.print(menu_line, style="green")  # View - green
            elif line.startswith("3."):
                console.print(menu_line, style="yellow")  # New - yellow
...
def view_recipes():
    ...
    if len(all_recipes) > 5:
        ...
        if search_choice == 'y':
            search_term = Prompt.ask("[yellow]Enter search term (recipe name)[/yellow]").strip()

            if search_term:
                # Filter recipes by name (case-insensitive, partial matching)
                filtered_recipes = [
                    (filename, recipe_data) for filename, recipe_data in all_recipes
                    if search_term.lower() in recipe_data.get('name', filename.replace('.json', '')).lower()
                ]
    ...
    for i, (filename, recipe_data) in enumerate(filtered_recipes, 1):
        ...
        if search_term and search_term.lower() in recipe_name.lower():
            ...
            if start_idx != -1:
                ...
                display_name = (recipe_name[:start_idx] +
        ...
        try:
            ...
            if 1 <= choice_num <= len(recipe_data_list):
                ...
                if not Confirm.ask("\n[cyan]View another recipe?[/cyan]"):
                    ...
                    console.print("[bold cyan]═══ View Existing Recipes ═══[/bold cyan]\n")
                    if search_term:
                        console.print(f"[green]Showing {len(filtered_recipes)} recipe(s) matching '{search_term}':[/green]\n")
                    else:
                        console.print(f"[green]Found {len(filtered_recipes)} recipe(s):[/green]\n")
                    console.print(recipes_table)
            else:
                console.print(f"[red]Please enter a number between 1 and {len(recipe_data_list)}[/red]")

        except ValueError:
            console.print("[red]Please enter a valid number, 's' to search again, or 'q' to quit[/red]")
...
def perform_recipe_calculation(recipe_data):
    ...
    console.print(f"[dim](You're making {scaling_factor:.1f}x the original recipe)[/dim]\n")

    # Calculate scaled amounts
    console.print("[bold yellow]Your Calculated Recipe:[/bold yellow]")

    scaled_table = Table(show_header=True, header_style="bold magenta", title="Scaled Recipe", box=None)
...
def delete_recipe():
    ...
    # Check if recipes directory exists
    if not os.path.exists(recipes_dir):
        console.print("[yellow]No recipes directory found. No recipes to delete![/yellow]")
        return
    ...
    console.print(f"[yellow]Found {len(recipe_files)} recipe(s) to choose from:[/yellow]\n")
    ...
    while True:
        try:
            ...
            if 1 <= choice_num <= len(recipe_data_list):
                ...
                if Confirm.ask(f"[red]Are you absolutely sure you want to delete '{recipe_name}'?[/red]"):
                    try:
                        os.remove(filepath)
                        ...
                        console.print(f"[bold red]Error deleting recipe: {e}[/bold red]")
                        return
                else:
                    console.print(f"[green]Delete cancelled. Recipe '{recipe_name}' was not deleted.[/green]")
